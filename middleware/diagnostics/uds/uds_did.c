...
static UdsDidResult read_value(void *context, uint16_t did, uint8_t *data,
...
void uds_did_registry_init(UdsDidRegistry *registry,
    ...
    set_entry(&registry->entries[0], UDS_DID_SOFTWARE_VERSION,
              &source->software_version, 64U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[1], UDS_DID_HARDWARE_VERSION,
              &source->hardware_version, 64U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[2], UDS_DID_BOOTLOADER_VERSION,
              &source->bootloader_version, 64U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[3], UDS_DID_SERIAL_NUMBER,
              &source->serial_number, 32U, UDS_DID_SESSION_ALL_MASK, 1U);
    set_entry(&registry->entries[4], UDS_DID_DEVICE_IDENTITY,
              &source->device_identity, 64U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[5], UDS_DID_CANOPEN_NODE_ID,
              &source->canopen_node_id, 1U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[6], UDS_DID_CAN_BITRATE,
              &source->can_bitrate, 4U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[7], UDS_DID_DIAGNOSTIC_STATUS,
              &source->diagnostic_status, 64U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[8], UDS_DID_FIRMWARE_STATUS,
              &source->firmware_status, 64U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[9], UDS_DID_RESET_CAUSE,
              &source->reset_cause, 4U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[10], UDS_DID_WATCHDOG_STATUS,
              &source->watchdog_status, 64U, UDS_DID_SESSION_ALL_MASK, 1U);
    set_entry(&registry->entries[11], UDS_DID_CAN_ERROR_COUNTERS,
              &source->can_error_counters, 32U, UDS_DID_SESSION_ALL_MASK, 0U);
    set_entry(&registry->entries[12], UDS_DID_CANOPEN_ERROR_STATE,
              &source->canopen_error_state, 4U, UDS_DID_SESSION_ALL_MASK, 0U);
}
